...
client.once(Events.ClientReady, async () => {
  console.log(`🤖 Logged in as ${client.user.tag}! Cập nhật Full tính năng cho LS-AntiCheat (30k)...`);
  ...
  try {
    ...
    await refreshChannel(channels.get("1542479128534716438"), async (ch) => {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          {
            name: "🛡️ 1. LS-AntiCheat (Hệ Thống Chống Hack Đa Năng)",
            value: 
              "• **⚔️ Check PvP & Utility:** AutoPotion, AutoFish, AutoEat, ShieldHit, ShieldSprint, ShieldInstantTap.\n" +
              "• **🎒 Check Inventory:** InvMove, InvHit, InvRotate, InvSprint, MultiInteraction, PerfectExit, thống kê AutoClicker (AverageClickHeuristic, Kurtosis/Skewness).\n" +
              "• **📡 Check BadPacket & Combat:** Animation (NoSwing / Forcefield), EqualRotations, PerfectRotation (Aimbot), IllegalPitch.\n" +
              "• **🎭 Fake Info & Obfuscator:** DamageIndicators Spoof (giả lập máu ảo chống hack soi máu), Max Health Spoof, EnchantmentHider (giấu phù phép), DurabilityHider (giấu độ bền giáp).\n" +
              "• **Hỗ trợ:** Paper / Purpur / Folia (1.16 - 1.21+)\n" +
              "• **Giá:** `30.000 VNĐ` *(Full trọn gói 4 Module)*"
          },
          {
        ...
        .addFields(
          {
            ...
            value: 
              "• 🛡️ **LS-AntiCheat (Full 4 Module):** `30.000đ`\n" +
              "• 🎁 **LS-GiftCode:** `30.000đ`\n" +
    ...
    console.log("🎉 ĐÃ CẬP NHẬT FULL TÍNH NĂNG LS-ANTICHEAT (30K) XONG 100%!");
    await cleanupAndExit(0);
  } catch (err) {
